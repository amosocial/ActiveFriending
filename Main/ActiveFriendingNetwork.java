...
public class ActiveFriendingNetwork extends Network{
	...
	public void get_rrsets(ArrayList<ArrayList<Integer>> rrsets, int size, Random rnd)
	{
		...
		for(int i=0;i<size;i++)
		{
			...
			get_rrset(this.neighbor_reverse,rrset,rnd, cfriends);
	...
	public void get_rrset(ArrayList<ArrayList<Integer>> re_neighbor,ArrayList<Integer> rrset, Random rnd, ArrayList<Integer> cfriends)
	{
		int centerIndex = t;
		switch(this.type)
		{
			...
			case "WC":
				break;
			case "LT":
				re_spreadOnceLT(re_neighbor, centerIndex, rrset, rnd, cfriends);
				break;
			default:
				System.out.print("Invalid model");
		}
	...
	public void re_spreadOnceLT(
			ArrayList<ArrayList<Integer>> re_neighbor, int cindex, ArrayList<Integer> rrset, Random rnd, ArrayList<Integer> cfriends)
	{
		rrset.add(cindex);
		while(true)
		{
			...
			cindex=re_neighbor.get(cindex).get((int) Math.floor(rnd.nextFloat()*re_neighbor.get(cindex).size()));
	...
	public double spreadOnce_invitation_set(ArrayList<Integer> invitation_set)
	{
		...
		while(newActive.size()>0)
		{
			...
			spreadOneRound_invitation_set(this.neighbor, newActive,state, invitation_set);
			
		}
		return 0;
	}
	
	public void spreadOneRound_invitation_set(ArrayList<ArrayList<Integer>> relationship, ArrayList<Integer> newActive, ArrayList<Boolean> state, ArrayList<Integer> invitation_set)
	{
